...
* [x] `sqlc` available for step 3 regeneration. Note: `make sqlc-gen` only covers
      `pkg/calendar/store`; the ada store was regenerated by hand with the pinned
      `SQLC_VERSION` (drift, see below).
...
Steps 1–5 are complete on `fix/ada-trends-bottle-amount-oz`. Step 6 (live verification)
is **pending deploy** — it cannot run until the engine image carrying this change is
released.

`TestFeedingEvents_BottleWeekReproduction` pins the defect: with `bottleSegOz` removed it
reproduces the reported payload exactly (14/10/14/9, grand 47) and with it in place reads
19/18/19/12, grand 68. The new `GetTodayFeedingAggregates.total_oz` was checked against
prod before commit — unchanged for the current bedtime day (15.0 oz / 5 feeds) and +1.00
oz across all history, which is the single `mixed` row it was meant to recover.

## Drift Observations

1. **`make sqlc-gen` covers only `pkg/calendar/store`.** The ada store
   (`services/engine/processors/ada/store`) has its own `sqlc.yaml` but no make target, so
   regeneration is a hand-typed `go run` — against the "make for everything" standard and
   an easy way to drift off the pinned `SQLC_VERSION`.
2. **`handleFeedingLogged` persists an internally inconsistent bottle row for mixed
   bottles**: `amount_oz` is written verbatim from the event, which sends only the split
   for a mixed bottle, leaving `amount_oz = 0` alongside a populated split. Read-side
   reconciliation now covers it, but the write path should populate `amount_oz` from the
   split (as `handleFeedingLoggedPast` and the edit path already do) so the column means
   one thing everywhere.
3. **PLAN-0039 is absent from the `docs/plans/README.md` archived table** despite being
   archived — `make docs-index` appears not to have been run when it landed.
